...
#RUN run_vm.sh -serial null -daemonize && \
#    wait_ssh.sh && \
#    ssh root@localhost 'wget https://toltec-dev.org/thirdparty/bin/wget-v1.21.1-1' && \
#    save_vm.sh
...
RUN mkdir -p /opt && \
    git clone https://github.com/timower/rM2-stuff.git /opt/rm2-stuff && \
    git clone https://github.com/FluffLescure/rM-bootstrap.git /opt/rm2-stuff/bs &&\
    cd /opt/rm2-stuff && git reset --hard $rm2_stuff_commit
WORKDIR /opt/rm2-stuff
...
COPY --from=rm2fb-host /opt/rm2-stuff/bs/wget-v1.21.1-1 /opt/rm2fb
...
RUN run_vm.sh -serial null -daemonize && \
    wait_ssh.sh && \
    ssh root@localhost 'rm /usr/bin/wget' && \
    ssh root@localhost 'chmod +x wget-v1.21.1-1' && \
    ssh root@localhost 'mv ./wget-v1.21.1-1  /usr/bin/wget' && \
    ssh root@localhost 'wget http://raw.githubusercontent.com/UniqueName992/rm2-bootstrap-temp/main/bootstrap' && \
    ssh root@localhost 'echo "a94ddad2aae1b560f5ff168dc309d4c4e39a4723136010253c31cdf3c5c5faad  bootstrap" | sha256sum -c && bash bootstrap'&& \
    ssh root@localhost 'systemctl mask remarkable-fail' && \
    ssh root@localhost 'source ~/.bashrc' && \
    ssh root@localhost 'wget https://raw.githubusercontent.com/UniqueName992/rm2-bootstrap-temp/main/xochitl_0.0.0-17_rmall.ipk' && \
    ssh root@localhost '/opt/bin/opkg remove xochitl'&&\
    ssh root@localhost '/opt/bin/opkg install xochitl_0.0.0-17_rmall.ipk' && \
    ssh root@localhost '/opt/bin/opkg flag hold xochitl' && \
    ssh root@localhost '/opt/bin/opkg install rm2fb-client' || true && \
    ssh root@localhost '/opt/bin/opkg flag hold rm2fb-client' && \
    ssh root@localhost 'wget https://github.com/timower/rM2-stuff/releases/download/v0.1.1/rm2fb.ipk' && \
    ssh root@localhost '/opt/bin/opkg install rm2fb.ipk' && \
    save_vm.sh

RUN run_vm.sh -serial null -daemonize && \
    wait_ssh.sh && \
    ssh root@localhost 'systemctl unmask remarkable-fail' && \
    echo " " | ssh-keygen -t rsa -b 4096 -f /root/.ssh/id_rsa && \
    cat ~/.ssh/id_rsa.pub | ssh root@localhost "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys" && \
    ssh root@localhost 'rm /usr/sbin/usermod' && \
    save_vm.sh

RUN sed -i 's/\/home\/root/\/opt\/lib/g' /opt/bin/run_xochitl.sh && \
    sed -i "11i ssh -o StrictHostKeyChecking=no root@localhost 'systemctl stop rm2fb'" /opt/bin/run_xochitl.sh
